...
SESSION_NAME="${1:-hw_timing_safe_manual}"
LOG_PATH="${2:-plans/logs/${SESSION_NAME}.log}"
KERNEL_FREQ_MHZ="${3:-250}"

cd /users/arashs/U280_CRC_Accelerator
...
echo "[CONFIG $(date -Is)] platform=${PLATFORM} kernel_freq_mhz=${KERNEL_FREQ_MHZ}"
...
echo "[BUILD HW $(date -Is)] make build TARGET=hw PLATFORM=${PLATFORM} VPP_LDFLAGS=--kernel_frequency ${KERNEL_FREQ_MHZ}"
make build TARGET=hw PLATFORM="${PLATFORM}" VPP_LDFLAGS="--kernel_frequency ${KERNEL_FREQ_MHZ}"
