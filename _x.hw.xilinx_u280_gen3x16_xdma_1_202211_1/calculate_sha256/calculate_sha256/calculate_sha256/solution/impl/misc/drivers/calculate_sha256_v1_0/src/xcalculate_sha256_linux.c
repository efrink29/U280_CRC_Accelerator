...
typedef struct {
    ...
    XCalculate_sha256_uio_map maps[ MAX_UIO_MAPS ];
} XCalculate_sha256_uio_info;

/***************** Variable Definitions **************************************/
static XCalculate_sha256_uio_info uio_info;
...
static int uio_info_read_name(XCalculate_sha256_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCalculate_sha256_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCalculate_sha256_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCalculate_sha256_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCalculate_sha256_Initialize(XCalculate_sha256 *InstancePtr, const char* InstanceName) {
	XCalculate_sha256_uio_info *InfoPtr = &uio_info;
